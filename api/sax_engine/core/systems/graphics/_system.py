...
_active_context: Dict[ContentEntity, AutoGraphicsComponent] = None

_fps_counter: RateTracker = RateTracker()
_fps_last_measurement: float = time()
on_rate_measured: EventDelegate[float] = EventDelegate("Graphics system FPS measured!")
...
def get_camera_setup(n_split: int=0) -> Iterator[RenderTarget]:
    # splits the screen into equal sized parts
    assert 0 <= n_split <= 2
...
def update():
    ...
    if now >= (_fps_last_measurement + 1):
        ...
        on_rate_measured(_fps_counter.rate)
